...
// 🧵 Liste des conversations utilisateur (derniers messages + statut lu/non-lu)
...
export default function ConversationList() {
  ...
  // 📥 Récupération des conversations dès qu'on a l'userId
  useEffect(() => {
    if (!userId) return

    const fetchConversations = async () => {
      // 🔎 Récupère tous les messages liés à l'utilisateur
  ...
  // 🧭 ID de la conversation active dans l'URL
  const currentPeerId = Array.isArray(router.query.id)
    ? router.query.id[0]
    : router.query.id

  // 🗑️ Suppression des messages avec un utilisateur donné
  ...
  return (
    <ul className="space-y-3">
      {conversations.map((conv) => (
        <li
          key={conv.peerId}
          className="relative group bg-white rounded-lg shadow px-4 py-3 transition hover:bg-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between"
        >
          <Link
...
<QuestionsWidget
  scope="global" // pas de variable locale => scope sûr et compilable
