...
// ------------------------------------------------------------------------------------
// Objet : Tâche CRON (appelée par Supabase Scheduled Jobs) pour purger automatiquement
...
//      (ex: limit 100 par exécution, pour éviter un batch énorme).
//   4) Pour chaque user_id : même logique que purge-profiles-avatar-default.ts :
//        - suppression des fichiers avatars contenant l'uid
//        - tentative d'appel admin_delete_account_hard(user_id)
...
export default async function handler(
  ...
  // 2) Récupérer les comptes à purger depuis la vue d’audit
  //    ⚠️ On cible bien le schéma "admin".
  const { data: rows, error: fetchErr } = await (admin as any)
    ...
    // et on limite à 100 par exécution pour rester raisonnable.
  ...
  if (targets.length === 0) {
    res.status(200).json({ ok: true, deleted_ids: [], failed_ids: [], scanned: 0 });
    return;
  }

  // 3) Pour chaque utilisateur cible : même logique que self-delete / purge manuelle
